//旧缓存数据只存储了id
const CACHE_KEY_OLD = 'faq-history'
//新缓存数据还包括配置
const CACHE_KEY = 'faq-history-v2'
...
function migrateOldData() {
  let oldList = JSON.parse(localStorage.getItem(CACHE_KEY_OLD) || '[]') || []
  let timeId = oldList.length + 1
  let newList = oldList.map(function(id) {
    return {
      optionValue: id,
...
function updateHistory(optionValue) {
  ...
  let index = findTarget(list, optionValue)
  if (index >= 0) {
...
function loadHistoryList(idToUser = {}) {
  let localList = getHistoryList()
  let list = localList.filter(function(item) {
    let user = idToUser[item.optionValue]
    item.user = user
    return !!user
  })
